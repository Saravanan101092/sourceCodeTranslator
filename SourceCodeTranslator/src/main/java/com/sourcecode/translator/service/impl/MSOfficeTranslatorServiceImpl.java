...
@Service
public class MSOfficeTranslatorServiceImpl implements MSOfficeTranslatorService {
	@Autowired
	...
	public File processExcelFile(MultipartFile file, Workbook workbook, String sourceLan, String targetLan)
		...
		for (int i = 0; i < noOfSheets; i++) {
				...
				while (cellIterator.hasNext()) {
					...
					if (cell.getCellType() == 1) {
						...
						try {
							...
							log.info("Error while translating :" + text);
						}
					}
				}
			}
			...
			try {
				translatedName = this.googleApiClient.simpleTranslate(sheetName, sourceLan, targetLan);
			} catch (Exception e) {
				log.info("Error while translating :" + sheetName);
			}
	...
	public File processDocFile(MultipartFile file, String sourceLan, String targetLan) throws Exception {
		...
		for (int i = 0; i < range.numParagraphs(); i++) {
			...
			for (int j = 0; j < para.numCharacterRuns(); j++) {
				...
				if ((tempString.length() != 1) || (tempString.matches("/^[A-z]+$/"))) {
					if ((tempString != null) && (tempString.contains(".")) && (!tempString.equalsIgnoreCase(""))
							&& (!tempString.equalsIgnoreCase(" "))) {
						String[] textStrings = tempString.split("\\.");
						for (int k = 0; k < textStrings.length; k++) {
							try {
								String translated = this.googleApiClient.simpleTranslate(textStrings[k], sourceLan,
										targetLan);
								tempString = tempString.replace(textStrings[k], translated);
							} catch (Exception e) {
								log.info("Error while translating :" + textStrings[k]);
							}
						}
					} else if ((tempString != null) && (!tempString.equalsIgnoreCase(""))
							&& (!tempString.equalsIgnoreCase(" "))) {
						try {
							...
							log.info("Error while translating :" + tempString);
						}
					}
	...
	public File processDocFilesText(MultipartFile file, String sourceLan, String targetLan) throws Exception {
		...
		if (text.contains(".")) {
			...
			for (int j = 0; j < textStrings.length; j++) {
				try {
					...
					log.info("Error while translating :" + textStrings[j]);
				}
			}
		} else {
			try {
					...
					for (XWPFParagraph p : cell.getParagraphs()) {
						for (XWPFRun r : p.getRuns()) {
							...
							if ((text != null) && (text.contains("."))) {
								...
								for (int j = 0; j < textStrings.length; j++) {
									try {
										String translated = this.googleApiClient.simpleTranslate(textStrings[j],
												sourceLan, targetLan);
										text = text.replace(textStrings[j], translated);
								...
								try {
									String translated = this.googleApiClient.simpleTranslate(text, sourceLan,
											targetLan);
									text = text.replace(text, translated);
									r.addBreak(BreakType.TEXT_WRAPPING);
									r.setText(text, 0);
